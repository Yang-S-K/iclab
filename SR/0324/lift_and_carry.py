...
ROUTE_PLAN_LAYER_FORE      = [2000,0,0,4   ]
ROUTE_PLAN_LAYER_FIVE      = [-300,-1000,8,8]
...
UP_WARNING_DISTANCE        = 8                    #上板危險距離
DOWN_WARNING_DISTANCE      = 1                      #下板危險距離
...
#LC主策略
    ...
    def main(self,strategy):
        ...
        if self.layer <4:
            ...
            send.sendHeadMotor(2,self.head_Vertical-50,100)#垂直
        ...
        if motion == 'ready_to_lc' or motion == 'continue_to_lc':
            ...
            if self.layer < 4:
                if GND_BOARD_LC and self.layer == 1:
                    ...
                    send.sendBodySector(211)          #上板前站姿調整
                    while not send.execute:
                ...
                if BOARD_GND_LC and self.layer == 6:
                    ...
                    send.sendBodySector(212)          #下板前站姿調整
                    while not send.execute:
    ...
    #邊緣判斷,回傳機器人走路速度與走路模式
        if ((self.distance[0] < GO_UP_DISTANCE+8) and (self.distance[1] < GO_UP_DISTANCE+6) and\
           ...
           self.state = "下底板"
           return 'ready_to_lc'
        elif ((self.distance[0] < GO_DOWN_DISTANCE) and (self.distance[1] < GO_DOWN_DISTANCE) and\
           (self.distance[2] < GO_DOWN_DISTANCE) and (self.distance[3] < GO_DOWN_DISTANCE) and\
           (self.distance[4] < GO_DOWN_DISTANCE)and (self.distance[5] < GO_DOWN_DISTANCE)and self.layer >=4):
           #上板
           self.state = "下板"
           return 'ready_to_lc'
        else:
