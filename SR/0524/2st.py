...
ROUTE_PLAN_LAYER_ONE       = [0,-1500,0,0]
ROUTE_PLAN_LAYER_TWO       = [1000,0,4,0,  0,1000,0,0]# 
ROUTE_PLAN_LAYER_TREE      = [0,-1600,0,9]
ROUTE_PLAN_LAYER_FORE      = [1000,0,0,3,   -300,1000,-8,4]
ROUTE_PLAN_LAYER_FIVE      = [500,0,0,0]
...
#LC主策略
        ...
        if motion == 'ready_to_lc' or motion == 'continue_to_lc':
            ...
            if self.layer < 4:
                if GND_BOARD_LC and self.layer == 1:
                    ...
                    rospy.sleep(1.5)
                    rospy.loginfo("準備上板\033[K")
                    send.sendBodySector(525)          #上板前站姿調整
                    while not send.execute:
                        rospy.logdebug("上板前姿勢\033[K")
                    rospy.sleep(1.5)
                    send.execute = False                   #微調站姿延遲
                    ...
                    rospy.sleep(1.5)
                # if self.layer==3:
                #     send.sendBodyAuto(17000,0,0,0,2,0)    #上板步態
                # else:                    
    ...
    #邊緣判斷,回傳機器人走路速度與走路模式
        ...
        elif (self.distance[0] < GO_UP_DISTANCE+4) and (self.distance[1] < GO_UP_DISTANCE+2) and\
           (self.distance[2] < GO_UP_DISTANCE+1) and (self.distance[3] < GO_UP_DISTANCE+1) and\
           (self.distance[4] < GO_UP_DISTANCE+2)and (self.distance[5] < GO_UP_DISTANCE+4) and self.layer < 4:
           #上板
           self.state = "上板"
           return 'ready_to_lc'        
        elif ((self.distance[2] < GO_DOWN_DISTANCE+15) and  (self.distance[3] < GO_DOWN_DISTANCE+15)) and\
