...
BASE_CHANGE                = 200                   
#上下板前進量
LCUP                       = 15000                 #上板 Y_swing = 7,Period_T = 840,OSC_LockRange = 0.4,BASE_Default_Z = 8,BASE_LIFT_Z = 3.2
...
ROUTE_PLAN_LAYER_ONE       = [0,1500,1,0]
ROUTE_PLAN_LAYER_TWO       = [300,1300,-3,4,    500,500,0,3]#
ROUTE_PLAN_LAYER_TREE      = [0,500,-3,4]
ROUTE_PLAN_LAYER_FORE      = [-300,1000,-8,0]
ROUTE_PLAN_LAYER_FIVE      = [-300,-1000,8,0,   1000,0,0,0]
ROUTE_PLAN_LAYER_SIX       = [0,0,0,0]
ROUTE_PLAN_LAYER_SEVEN     = [0,0,0,0]

# ROUTE_PLAN_LAYER_ONE       = [0,1000,0,4, 1500,0,0,12,    -300,1000,-8,5]
# ROUTE_PLAN_LAYER_TWO       = [1500,0,0,5]
# ROUTE_PLAN_LAYER_TREE      = [0,500,0,3]
# ROUTE_PLAN_LAYER_FORE      = [1500,0,0,5,   -300,-1000,8,6]
# ROUTE_PLAN_LAYER_FIVE      = [-300,-1000,8,0,   1000,0,0,0]
# ROUTE_PLAN_LAYER_SIX       = [0,0,0,0]
# ROUTE_PLAN_LAYER_SEVEN     = [0,0,0,0]
ROUTE_PLAN = [
...
#----------#                       右腳           左腳
#                              左 ,  中,  右|  左,  中,   右S
FOOT                       = [93 , 116, 136, 157, 185, 201]
HEAD_HORIZONTAL            = 2048                  #頭水平
HEAD_VERTICAL              = 1340                #頭垂直 #down 2750
##判斷值
FOOTBOARD_LINE             = 210                   #基準線
UP_WARNING_DISTANCE        = 0                    #上板危險距離
DOWN_WARNING_DISTANCE      = 0                      #下板危險距離
GO_UP_DISTANCE             = 3                    #上板距離
GO_DOWN_DISTANCE           = 5                     #下板距離
...
#LC主策略
        ...
        #開啟LC策略
            if self.layer < 7:
                if self.walkinggait_stop and self.first_in:
                    ...
                    send.execute = False
                    if STAND_CORRECT_LC:
                        send.sendBodySector(21)             #LC基礎站姿調整磁區
                        while not send.execute:
        ...
        if motion == 'ready_to_lc' or motion == 'continue_to_lc':
            ...
            if self.layer < 4:
                if GND_BOARD_LC and self.layer == 1:
                    ...
                    send.sendBodySector(209)          #上板前站姿調整
                    while not send.execute:
                    ...
                    rospy.sleep(1.5)
                if self.layer==2:
                    send.sendBodyAuto(14000,0,0,0,2,0)    #上板步態
                else:                            
                ...
                if BOARD_GND_LC and self.layer == 6:
                    ...
                    rospy.sleep(2)
                    rospy.loginfo("準備下板\033[K")
                    send.sendBodySector(207)          #下板前站姿調整
                    while not send.execute:
                        rospy.logdebug("下板前姿勢\033[K")
                    rospy.sleep(2)
                    send.execute = False               #微調站姿延遲
    ...
    def edge_judge(self):
    #邊緣判斷,回傳機器人走路速度與走路模式
        if ((self.distance[0] < GO_UP_DISTANCE+50) and (self.distance[1] < GO_UP_DISTANCE+20) and\
           (self.distance[2] < GO_UP_DISTANCE+14) and (self.distance[3] < GO_UP_DISTANCE+14) and\
           (self.distance[4] < GO_UP_DISTANCE+20)and (self.distance[5] < GO_UP_DISTANCE+50)) and\
        ...
        elif (self.distance[0] < GO_UP_DISTANCE+0) and (self.distance[1] < GO_UP_DISTANCE+0) and\
           (self.distance[2] < GO_UP_DISTANCE+0) and (self.distance[3] < GO_UP_DISTANCE+0) and\
           (self.distance[4] < GO_UP_DISTANCE+0)and (self.distance[5] < GO_UP_DISTANCE+0) and self.layer < 4:
           #上板
           self.state = "上板"
           return 'ready_to_lc'        
        elif ((self.distance[0] < GO_DOWN_DISTANCE+15) and  (self.distance[5] < GO_DOWN_DISTANCE+15)) and\
    ...
    def theta_change(self):
        ...
        #     slope = edge.slope  # 直接使用當前斜率
        #     rospy.logwarn(f"【等待平滑啟動】當前slope: {slope}")
        # else:
        #     # 平滑 slope 變化
        #     if not hasattr(self, 'smooth_slope'):
        #         self.smooth_slope = edge.slope  # 初始化平滑斜率
        #     else:
        #         self.smooth_slope = (1 - alpha) * self.smooth_slope + alpha * edge.slope  # 避免突變
        ...
        if ROUTE_PLAN_FLAG:
            for t in range(len(ROUTE_PLAN[now_layer-1])//4):                
                ...
                while (end-start) < ROUTE_PLAN[now_layer-1][3+4*t]:
                    ...
                    rospy.loginfo(f"Now_layer:{now_layer}\033[K")
                    rospy.loginfo(f"{ROUTE_PLAN[now_layer-1][0+4*t]},{ROUTE_PLAN[now_layer-1][1+4*t]},{ROUTE_PLAN[now_layer-1][2+4*t]},{ROUTE_PLAN[now_layer-1][3+4*t]}\033[K")
