...
ROUTE_PLAN_LAYER_ONE       = [0,-700,-3,0]
ROUTE_PLAN_LAYER_TWO       = [0,1300,3,0]
ROUTE_PLAN_LAYER_TREE      = [500,-1500,8,0]
ROUTE_PLAN_LAYER_FORE      = [-300,-1000,8,7]
ROUTE_PLAN_LAYER_FIVE      = [-300,-1000,8,0]
ROUTE_PLAN_LAYER_SIX       = [0,1200,0,0]
ROUTE_PLAN = [
...
UP_WARNING_DISTANCE        = 20                    #上板危險距離
DOWN_WARNING_DISTANCE      = 7                      #下板危險距離
...
#LC主策略
    ...
    #邊緣判斷,回傳機器人走路速度與走路模式
        if ((self.distance[0] < GO_UP_DISTANCE+8) and (self.distance[1] < GO_UP_DISTANCE+6) and\
           ...
           (self.distance[4] < GO_DOWN_DISTANCE+3)and (self.distance[5] < GO_DOWN_DISTANCE+3)and self.layer == 6 and\
            min(self.distance)>DOWN_WARNING_DISTANCE):
           self.state = "下底板"
           return 'ready_to_lc'
        elif ((self.distance[0] < GO_DOWN_DISTANCE+3) and (self.distance[1] < GO_DOWN_DISTANCE+3) and\
           (self.distance[2] < GO_DOWN_DISTANCE+3) and (self.distance[3] < GO_DOWN_DISTANCE+3) and\
           (self.distance[4] < GO_DOWN_DISTANCE+3)and (self.distance[5] < GO_DOWN_DISTANCE+3)and self.layer >=4 and\
            min(self.distance)>DOWN_WARNING_DISTANCE):
           #上板
           self.state = "下板"
           return 'ready_to_lc'
