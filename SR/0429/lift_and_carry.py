...
ROUTE_PLAN_LAYER_TWO       = [1300,0,0,4,    -300,-1000,8,3]# 
ROUTE_PLAN_LAYER_TREE      = [0,0,3,3]
ROUTE_PLAN_LAYER_FORE      = [0,1200,-8,2]
ROUTE_PLAN_LAYER_FIVE      = [0,500,-8,0]
...
DOWN_WARNING_DISTANCE      = 10                      #下板危險距離
GO_UP_DISTANCE             = 25                    #上板距離
GO_DOWN_DISTANCE           = 25                     #下板距離
...
#LC主策略
    ...
    #邊緣判斷,回傳機器人走路速度與走路模式
        if ((self.distance[0] < GO_UP_DISTANCE+8) and (self.distance[1] < GO_UP_DISTANCE+6) and\
           ...
           self.state = "上板"
           return 'ready_to_lc'
        elif ((self.distance[0] < GO_UP_DISTANCE+5) and (self.distance[1] < GO_UP_DISTANCE+6) and\
           (self.distance[2] < GO_UP_DISTANCE+8) and (self.distance[3] < GO_UP_DISTANCE+8) and\
           (self.distance[4] < GO_UP_DISTANCE+6)and (self.distance[5] < GO_UP_DISTANCE+5) and\
            min(self.distance)>UP_WARNING_DISTANCE) and\
           ...
           self.state = "下板"
           return 'ready_to_lc'
        elif ((self.distance[0] < GO_DOWN_DISTANCE+10) and (self.distance[1] < GO_DOWN_DISTANCE+3) and\
           (self.distance[2] < GO_DOWN_DISTANCE+3) and (self.distance[3] < GO_DOWN_DISTANCE+3) and\
           (self.distance[4] < GO_DOWN_DISTANCE+3) and (self.distance[5] < GO_DOWN_DISTANCE+10)) and\
