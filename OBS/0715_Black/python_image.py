...
class Normal_Obs_Parameter: #計算各種深度
    ...
    #----------------Blue_DeepMatrix-----------------
            ...
            # self.b_left_deep    = deep.ba[2] if isinstance(deep.ba, (list, tuple)) and len(deep.ba) > 2 else 24
...
class Obs: #各種避障動作
    ...
    def crawl(self):
        ...
        time.sleep(8)
        while self.crawl_cnt < 6:    #count 6次            
        ...
        if (self.right_deep_sum > self.left_deep_sum) or self.image.line_at_left or self.line_at_left_single or self.door_at_right: #turn head 右轉
            ...
            while abs(self.image.deep_x) >= 2:
                ...
                if send.color_mask_subject_XMin[5][0] <80 and send.color_mask_subject_cnts[5] >= 1:
                    break
                if (self.temp_dx/self.image.deep_x) < 0:
                    break
                #sys.stdout.write("\033[H\033[J")
            ...
            self.image.calculate() 
            if (self.image.center_deep > 6):                   #turn head 左轉 前後修正 越大越遠
                while ( self.image.center_deep > 6 ):
                    ...
                    self.walk.move('small_forward') 
                    #sys.stdout.write("\033[H\033[J")
            elif ( self.image.center_deep < 5 ):                #turn head 左轉 前後修正 越大越遠 
                while ( self.image.center_deep < 5 ):
                    self.image.calculate()
                    self.walk.move('small_back') 
                    #sys.stdout.write("\033[H\033[J")
    ...
    def draw_function(self):        
        ...
        if self.image.YY_2:   
            ...
            send.drawImageFunction(3,0,160,160,0,240,255,255,255)#畫面中心 
            send.drawImageFunction(4,0,0,0,0,0,0,0,0)#dx                
            send.drawImageFunction(5,0,self.image.yellow_leftside,self.image.yellow_leftside,0,240,160,32,240)#左邊黃通
            send.drawImageFunction(6,0,self.image.yellow_rightside,self.image.yellow_rightside,0,240,160,32,240)#右邊黃通
            send.drawImageFunction(7,0,int(self.image.yellow_center),int(self.image.yellow_center),0,240,255,0,0)#中間黃通
        else:
            send.drawImageFunction(1,0,40,40,int(240-max(deep.aa[3:6])*10),240,192,192,192)#左邊深度
            send.drawImageFunction(2,0,280,280,int(240-max(deep.aa[27:30])*10),240,192,192,192)#右邊深度
            send.drawImageFunction(3,0,160,160,int(240-deep.aa[16]*10),240,128,42,42)#中間深度
            send.drawImageFunction(4,0,int(self.image.deep_x*10 if self.image.deep_x > 0 else (self.image.deep_x+32)*10),\
                               int(self.image.deep_x*10 if self.image.deep_x > 0 else (self.image.deep_x+32)*10),0,240,255,0,0)#dx                    
    ...
    def main(self):
        ...
        #=============================strategy=============================
            ...
            #         pass
                ...
                if self.image.deep_y < 24:
                    ...
                    if self.image.line_at_right or self.image.line_at_left:
        ...
        if not send.is_start :
            ...
            # rospy.loginfo("blue_cnt:", send.color_mask_subject_cnts[2])
            rospy.loginfo("dx: %s\033[2K", self.image.deep_x)
