    ...
    def move(self, action_id, z=0, sensor= 0):
        ...
        actions             = { 'stay'                  : {'x'      : STAY_X,     \
                                ...
                                'slope_fix'             : {'x'      : IMU_LEFT_X   if deep.slope > 0 else IMU_RIGHT_X,                   \
                                                           ...
                                                           'theta'  : 4 if deep.slope > 0 else -4},
                                ################################################
...
class Normal_Obs_Parameter: #計算各種深度
    ...
    #----------------Filter_matrix-------------------
            ...
            if send.color_mask_subject_cnts[1] == 2 and (self.yellow_rightside - self.yellow_leftside) > 0 and (not strategy.turn_head_flag):
                self.YY_2 = True   #打開兩個黃障礙旗標
                Y_XMAX1                = send.color_mask_subject_XMax[1][0]
                Y_XMin1               = send.color_mask_subject_XMin[1][0]
                Y_XMin2                = send.color_mask_subject_XMin[1][1]
...
LCRdeep          : {self.image.left_deep} {self.image.right_deep} {self.image.center_deep}\n\
#=================黃黃=================#\n\
...
class Obs: #各種避障動作    
    ...
    def red_door(self): #前後修正1 -> 修斜率 -> 前後修正2 -> 平移 -> 前後修正3 -> 趴下
        ...
        while send.color_mask_subject_YMax[5][0] > 130 and status.running: #red door 前後修正1 值越大離門越近 #離紅門太近了            
        ...
        # if (send.color_mask_subject_YMax[5][0] < 95) :     #red door 前後修正2 值越大離門越近 
        #     status.reddoor_state = "離紅門太遠"
        #     while send.color_mask_subject_YMax[5][0] < 95 and status.running: #離紅門太遠了                
    ...
    def turn_head(self):
        ...
        if (self.right_deep_sum > self.left_deep_sum) or self.image.line_at_left or self.line_at_left_single or self.door_at_right: #turn head 右轉            
            ...
            if (self.image.center_deep) > 8 :
                # while ( self.image.center_deep > 8) and (not self.image.center_deep == 24) and status.running and (not self.yb):
                while ( self.image.center_deep > 8)and status.running:                    
                    status.turnHead_state = "距離牆太遠"
                    self.walk.move('small_forward')                    
            # elif ( self.image.center_deep < 7) or (self.image.center_deep ==24)and (not self.yb):
            elif ( self.image.center_deep < 7) :
                # while (( self.image.center_deep < 7 ) or (self.image.center_deep == 24)) and status.running and (not self.yb):
                while ( self.image.center_deep < 7 )and status.running:                    
        ...
        elif (self.left_deep_sum > self.right_deep_sum) or self.image.line_at_right or self.line_at_right_single or self.door_at_left:            
            ...
            while abs(self.image.deep_x) >= 3.5 and status.running:                
                status.turnHead_state = "頭面向牆"
    ...
    def main(self):        
        ...
        #=============================strategy=============================
            ...
            if self.preturn_left:
                while abs(self.walk.get_imu()) < 40 and status.running:
