...
YELLOW_IMU_LEFT        = True #黃色與藍牆夠近時 走完轉頭先imu_fix
...
class Obs: #各種避障動作    
    ...
    def turn_head(self):
        ...
        if (self.right_deep_sum > self.left_deep_sum) or self.image.line_at_left or self.line_at_left_single or self.door_at_right or FORCE_TURN_RIGHT: #turn head 右轉            
            ...
            while abs(self.image.center_deep) <= 9 and status.running:                              
            ...
            # while abs(self.image.deep_x) >= 5 and status.running:
            while abs(deep.aa[18]) <= 9 and status.running:                 
                status.turnHead_state = "頭面向牆"
