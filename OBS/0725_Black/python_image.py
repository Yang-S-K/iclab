...
#===========================================     
CRMAX           = 80 # red door 前後修正3 值越大離門越近 #68
...
YELLOW_DX_BREAK_LEFT        = False #黃色夠近時 轉頭dx相反break 朝左走
YELLOW_DX_BREAK_RIGHT       = False #黃色夠近時 轉頭dx相反break 朝右走
...
class Obs: #各種避障動作    
    ...
    def red_door(self): #前後修正1 -> 修斜率 -> 前後修正2 -> 平移 -> 前後修正3 -> 趴下
        ...
        while send.color_mask_subject_YMax[5][0] > 100 and status.running: #red door 前後修正1 值越大離門越近 #離紅門太近了            
        ...
        if (send.color_mask_subject_YMax[5][0] < 75) :     #red door 前後修正2 值越大離門越近 
            status.reddoor_state = "離紅門太遠"
            while send.color_mask_subject_YMax[5][0] < 75 and status.running: #離紅門太遠了                
                self.walk.move('small_forward') 
                
        elif (self.image.red_y_max > 75) or self.image.b_center_deep == 0:   #red door  前後修正2 值越大離門越近 
            status.reddoor_state = "離紅門太近"
            while self.image.red_y_max > 75 or self.image.b_center_deep == 0 and status.running:                
                self.walk.move('small_back')
    ...
    def turn_head(self):
        ...
        if (self.right_deep_sum > self.left_deep_sum) or self.image.line_at_left or self.line_at_left_single or self.door_at_right: #turn head 右轉            
            ...
            if (self.image.center_deep) > 7 and (not self.yb):
        ...
        #=============================strategy=============================
            ...
            if self.image.at_reddoor_flag: #進紅門
                ...
                # if self.image.deep_y < 24:                    
                    ...
                    if (self.image.line_at_right or self.image.line_at_left) and self.image.YY_small and (self.image.y_deep_y < 5) and\
                        YELLOW_SMALL_TURNHEAD:
                        while ( abs(self.walk.get_imu()) > 5) :
                            self.walk.move('imu_fix')                        
                        
                        if self.image.center_deep <15 :
                            self.turn_head()
