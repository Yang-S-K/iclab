...
deep L R         : {deep.aa[14]} {deep.aa[18]}\n\
    ...
    def runThread(self):        
        ...
        while send.is_start:
            ...
            if strategy.reddoor_running or strategy.turn_head_flag:
                sys.stdout.write(f"sleep:0.1\n")
                time.sleep(0.1)
...
class Obs: #各種避障動作    
    ...
    def red_door(self): #前後修正1 -> 修斜率 -> 前後修正2 -> 平移 -> 前後修正3 -> 趴下
        ...
        elif (self.image.red_y_max > 125) or self.image.b_center_deep == 0:   #red door  前後修正2 值越大離門越近 
            status.reddoor_state = "離紅門太近"
            while self.image.red_y_max > 125 or self.image.b_center_deep == 0 and status.running:                
                self.walk.move('small_back')
