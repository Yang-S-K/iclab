...
class Normal_Obs_Parameter: #計算各種深度
    ...
    #----------------Filter_matrix-------------------
            ...
            # (self.yellow_rightside - self.yellow_leftside) > 0 and\
                ...
                # and max(send.color_mask_subject_size[1])>2000:                    
...
'''
    def blue_wall_x(self, face=""):
        ...
        # === 左牆右上角 ===
        if face == "right":
            indices_ymin_left = np.where(np.isclose(y_left, ymin_left))[0]
            if len(indices_ymin_left) > 0:
                idx_right_top_leftwall = indices_ymin_left[np.argmax(x_left[indices_ymin_left])]
            else:
                print("[警告] 找不到左牆右上角（face=right）")
                return 0
        elif face == "left":
            indices_xmax_left = np.where(np.isclose(x_left, xmax_left))[0]
            if len(indices_xmax_left) > 0:
                idx_right_top_leftwall = indices_xmax_left[np.argmin(y_left[indices_xmax_left])]
            else:
                print("[警告] 找不到左牆右上角（face=left）")
                return 0
        else:
        ...
        # === 右牆左上角 ===
        if face == "right":
            indices_xmin_right = np.where(np.isclose(x_right, xmin_right))[0]
            if len(indices_xmin_right) > 0:
                idx_left_top_rightwall = indices_xmin_right[np.argmin(y_right[indices_xmin_right])]
            else:
                print("[警告] 找不到右牆左上角（face=right）")
                return 0
        elif face == "left":
            indices_ymin_right = np.where(np.isclose(y_right, ymin_right))[0]
            if len(indices_ymin_right) > 0:
                idx_left_top_rightwall = indices_ymin_right[np.argmin(x_right[indices_ymin_right])]
            else:
                print("[警告] 找不到右牆左上角（face=left）")
                return 0
        else:
...
LCRdeep          : {self.left_deep} {self.right_deep} {self.center_deep}\n\
#=================黃黃=================#\n\
yellow_state     : {self.yellow_state}\n\
yellow_side      : {self.yellow_leftside} {self.yellow_rightside}\n\
...
bcnt             : {self.bcnt}\n\
blue_side        : {self.blue_leftside} {self.blue_rightside}\n\
...
#         sys.stdout.write(f"\
# test:{}\n")
        # self.image.blue_wall_x(face="left")
        # sys.stdout.flush()
...
class Obs: #各種避障動作
    ...
    def red_door(self): #前後修正1 -> 修斜率 -> 前後修正2 -> 平移 -> 前後修正3 -> 趴下
        ...
        #self.image.calculate()
        while self.image.red_y_max > 130 and status.running: #red door 前後修正1 值越大離門越近 #離紅門太近了
            #self.image.calculate()
        ...
        if (self.image.red_y_max < 95) :     #red door 前後修正2 值越大離門越近 
            ...
            while self.image.red_y_max < 95 and status.running: #離紅門太遠了
                #self.image.calculate()
                self.walk.move('small_forward') 
                #sys.stdout.write("\033[H\033[J")
        elif (self.image.red_y_max > 95) or self.image.b_center_deep == 0:   #red door  前後修正2 值越大離門越近 
            status.reddoor_state = "離紅門太近"
            while self.image.red_y_max > 95 or self.image.b_center_deep == 0 and status.running:
                #self.image.calculate()
                self.walk.move('small_back')
                #sys.stdout.write("\033[H\033[J")
    ...
    def turn_head(self):
        ...
        if (self.right_deep_sum > self.left_deep_sum) or self.image.line_at_left or self.line_at_left_single or self.door_at_right: #turn head 右轉
            ...
            if (self.image.center_deep) > 8 and (not self.yb):                   #turn head 右轉 前後修正 越大越遠
                while ( self.image.center_deep > 8) and (not self.image.center_deep == 24) and status.running and (not self.yb):
                    ...
                    self.walk.move('small_forward')
                    #sys.stdout.write("\033[H\033[J")
            elif ( self.image.center_deep < 7) or (self.image.center_deep ==24)and (not self.yb):                 #turn head 右轉 前後修正 越大越遠
                while (( self.image.center_deep < 7 ) or (self.image.center_deep == 24)) and status.running and (not self.yb):
                    #self.image.calculate()
            ...
            while abs(self.image.deep_x) >= 2 and status.running:
                #self.image.calculate()
            ...
            if (self.image.center_deep > 8) and (not self.yb):                   #turn head 左轉 前後修正 越大越遠
                while ( self.image.center_deep > 8 ) and (not self.image.center_deep==24) and status.running and (not self.yb):
                    ...
                    self.walk.move('small_forward') 
                    #sys.stdout.write("\033[H\033[J")
            elif (( self.image.center_deep < 7 ) or (self.image.center_deep == 24)) and (not self.yb):                #turn head 左轉 前後修正 越大越遠 
            ...
            while abs(self.image.deep_x) >= 4 and status.running:
                #self.image.calculate()
                status.turnHead_state = "頭面向牆"
    ...
    def main(self):
        ...
        #=============================strategy=============================
            ...
            if self.preturn_left:
                while abs(self.walk.get_imu()) < 65 and status.running:
            ...
            #         pass
                ...
                if self.image.yellow_move :
                ...
                elif ( self.image.left_deep < 15 ) and ( self.image.right_deep < 15 ) and ( self.image.center_deep < 15 ) and (max(send.color_mask_subject_size[1])<5000):
                    self.yb = True
                    ...
                    if (self.image.line_at_right or self.image.line_at_left) and self.image.YY_small and (self.image.y_deep_y < 5):
                        ...
                        if ( abs(self.walk.get_imu()) > 2) and (not self.imu_ok) :                   #IMU修正
                            while ( abs(self.walk.get_imu()) > 2) and (not self.imu_ok) and status.running:
                                ...
                                if abs(self.walk.get_imu()) < 2:        #轉頭策略
                                    if ( self.image.left_deep < 15 ) and ( self.image.right_deep < 15 ) :
                                    # and ( self.image.center_deep < 15 ): #左中右深度皆小於15
                                        self.turn_head()
                                        self.imu_ok = True
                                        break
                                    # elif (send.color_mask_subject_XMax[2][0] > 50) and (send.color_mask_subject_XMin[2][1] < 260) :
                                    #     self.deep_x = 0
                                    elif  self.image.deep_sum_l >= self.image.deep_sum_r and ( self.image.center_deep < 15 ):
                                        while abs(self.walk.get_imu()) < 20 and status.running and (not self.image.YY_2):    
                                            self.walk.move('turn_left_for_wall')
                                            #sys.stdout.write("\033[H\033[J")
                                            # rospy.loginfo('dx: %s', self.image.deep_x)
