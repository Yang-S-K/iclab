...
SLOPE_LEFT_TRANSLATE_Y          = 1700
SLOPE_LEFT_TRANSLATE_THETA      = 2
#===========================================
YY_WALKWAY            =     120 #黃黃通道大小
YY_ERRO               =     45 #黃黃通道中心與畫面中心誤差值
#===========================================     
...
class Walk(): #步態、轉彎、直走速度、IMU
    ...
    def move(self, action_id, z=0, sensor= 0):
        ...
        rospy.loginfo(f"{action_id}\033[K")
        rospy.loginfo(f"{self.total_movement}\033[K")
...
#--------------------fix to r--------------------
        ...
        if send.color_mask_subject_size[5][0] < 5000 :
            slope_angle = 0
        rospy.loginfo(f"{slope_angle}\033[K")
        return slope_angle
...
class Normal_Obs_Parameter: #計算各種深度
    ...
    #----------------Filter_matrix-------------------
            ...
            if send.color_mask_subject_cnts[1] == 2 and (self.yellow_rightside - self.yellow_leftside) > 0:
                ...
                rospy.loginfo('A: %s \033[K',(self.yellow_rightside - self.yellow_leftside))
                rospy.loginfo(f"A: {(self.yellow_rightside - self.yellow_leftside)} \033[K")
                ...
                if abs(self.yellow_rightside - self.yellow_leftside) < YY_WALKWAY:
                    ...
                    if self.yellow_leftside > 160 and self.yellow_rightside > 160: #黃線在右邊
                        ...
                        # self.deep_x = -20
                        # rospy.loginfo("rrrrr\033[K")
                        rospy.loginfo("黃線在右\033[K")
                        self.line_at_right = True 
                        self.line_at_left  = False

                    elif self.yellow_leftside < 160 and self.yellow_rightside < 160: #黃線在左邊
                        # self.deep_x = 20
                        rospy.loginfo('YY_XMAX1: %s \033[K', send.color_mask_subject_XMax[1][0])
                        rospy.loginfo('YY_XMin1: %s \033[K', send.color_mask_subject_XMin[1][0])

                        rospy.loginfo('YY_XMAX2: %s\033[K', send.color_mask_subject_XMax[1][1])
                        rospy.loginfo('YY_XMin2: %s\033[K', send.color_mask_subject_XMin[1][1])

                        # rospy.loginfo("lllll\033[K")
                        rospy.loginfo("黃線在左\033[K")
                        self.line_at_right = False
                    ...
                    if self.yellow_erro < YY_ERRO*-1:
                        self.deep_x = -20
                    elif self.yellow_erro > YY_ERRO:
                        self.deep_x = 20
                ...
                if self.yy:
                    ...
                    rospy.logwarn("dx = %s\033[K",self.deep_x)
                    rospy.loginfo("dx = %s\033[K",self.deep_x)
            rospy.loginfo(
                "b_cnt = %d, b_l_max = %d, b_r_min = %d \033[K",
...
class Obs: #各種避障動作
    ...
    def red_door(self): #前後修正1 -> 修斜率 -> 前後修正2 -> 平移 -> 前後修正3 -> 趴下
        ...
        while 1 :     
            ...
            if (self.image.red_x_min < 2 and self.image.red_x_max > 315) and send.color_mask_subject_size[5][0] > 5000: #紅門在眼前
                self.image.calculate()
                rospy.loginfo("xxxxxxxxxxxxxxxxxxxxxxxxx\033[K")
                rospy.loginfo("b_cnt = %d\033[K",send.color_mask_subject_cnts[2])
                ...
                if  self.translate:
                    ...
                    if (self.image.b_x_min < 2 and self.image.b_x_max > 40):
                        ...
                        # rospy.loginfo("333333333333333333333\033[K")
                        rospy.loginfo("紅門右平移 1B\033[K")
                        ...
                        # rospy.loginfo("4444444444444444444444\033[K")
                        rospy.loginfo("紅門左平移 1B\033[K")
                    else:
                        self.translate = True
                ...
                # elif (self.image.b_x_max == 0 and self.image.b_x_min == 0):
                    ...
                    if self.image.blue_rightside < 255 and self.image.blue_leftside < 35: #停在太右邊 blue_rightside調大
                        ...
                        self.walk.move('slope_left_translate')
                        self.translate = False
                        # rospy.loginfo("555555555555555\033[K")
                        rospy.loginfo("紅門左平移 2B\033[K")
                        rospy.loginfo("blue_rightside: %d\033[K", self.image.blue_rightside)
                        rospy.loginfo("blue_leftside: %d\033[K", self.image.blue_leftside)
                    # elif(send.color_mask_subject_XMax[2][0]>20 and send.color_mask_subject_XMin[2][1]>265):
                        ...
                        self.walk.move('slope_right_translate')
                        self.translate = False
                        # rospy.loginfo("666666666666666\033[K")
                        rospy.loginfo("紅門右平移 2B\033[K")
                        rospy.loginfo("blue_rightside: %d\033[K", self.image.blue_rightside)
                        rospy.loginfo("blue_leftside: %d\033[K", self.image.blue_leftside)
                    else:
                        self.translate = True
                    ...
                    self.image.calculate()
                    # rospy.loginfo("77777777777777777777\033[K")
                    rospy.loginfo("紅門夠大 0B \033[K")
    ...
    def crawl(self):
        ...
        while 1 :
            ...
            if self.translate :
                while abs(deep.slope) > 0.03 :
                    self.walk.move('slope_fix')
                    # rospy.loginfo('44444444444444444444444444444\033[K')
                    rospy.loginfo("修斜率\033[K")
            ...
            # elif (self.image.b_x_max == 0 and self.image.b_x_min == 0):
                ...
                if self.image.blue_rightside < 255 and self.image.blue_leftside < 35: #停在太右邊 blue_rightside調大
                    ...
                    # rospy.loginfo("555555555555555\033[K")
                    rospy.loginfo("紅門左平移 2B\033[K")
                    rospy.loginfo("blue_rightside: %s\033[K", self.image.blue_rightside)
                    rospy.loginfo("blue_leftside: %s\033[K", self.image.blue_leftside)
                # elif(send.color_mask_subject_XMax[2][0]>20 and send.color_mask_subject_XMin[2][1]>265):
                elif self.image.blue_rightside > 255 and self.image.blue_leftside > 35:  #停在太左邊 blue_leftside調小 #290 65
                    self.image.calculate()
                    # self.blue_at_left = True
        ...
        if((self.image.red_y_max) < CRMIN):            #離紅門太遠
            while(self.image.red_y_max < CRMIN):          
                ...
                self.walk.slope()
                self.walk.move('small_forward')
                rospy.loginfo("CRMIN: %s\033[K", self.image.red_y_max)
        elif(self.image.red_y_max > CRMAX):          #離紅門太近
            while(self.image.red_y_max > CRMAX):         
                self.image.calculate()
                rospy.loginfo("CRMAX: %s\033[K", self.image.red_y_max)
                self.walk.slope()
                self.walk.move('small_back')  
        ...
        while self.crawl_cnt < 9:   #cnt3數到7(4次)
            self.image.calculate()
            rospy.loginfo("blue_deep = %s\033[K", self.image.deep_y)
    ...
    def turn_head(self):
        ...
        if not self.image.line_at_right and not self.image.line_at_left: 
            ...
            rospy.loginfo(f"{self.line_at_right_single}\033[K")
            send.sendHeadMotor(1,2599,180) #頭往左轉
            send.sendHeadMotor(2,HEAD_HEIGHT+150,180)
            send.sendHeadMotor(1,2599,180)
            send.sendHeadMotor(2,HEAD_HEIGHT+150,180)
            ...
            rospy.loginfo(f"{self.line_at_left_single}\033[K")
        ...
        if (self.right_deep_sum > self.left_deep_sum) or self.image.line_at_left or self.line_at_left_single or self.door_at_right: #turn head 右轉
            ...
            if abs(self.walk.get_imu()) < 85:                     #turn head 旋轉角度  右轉 越大轉越多
                while abs(self.walk.get_imu()) < 85:
                    ...
                    rospy.loginfo("身體面相右，頭往左轉看牆\033[K")
            ...
            if (self.image.b_center_deep > 6):                   #turn head 左轉 前後修正 越大越遠
                while ( self.image.b_center_deep > 6 ):
                    rospy.loginfo(f"{self.image.b_center_deep}\033[K")
            ...
            if abs(self.walk.get_imu()) < 90:                           #turn head 旋轉角度  左轉 越大轉越多
                while abs(self.walk.get_imu()) < 90:
                    ...
                    # rospy.loginfo('llllllllllllllllllllllllllll\033[K')
                    rospy.loginfo("身體面相左，頭往右轉看牆\033[K")
    ...
    def main(self):
        if send.is_start : #策略主指撥開關
            ...
            self.image.calculate() #計算障礙物的各種參數(深度、左右權重、dx...)
                        # self.image.calculate()
            rospy.loginfo('dx: %s\033[K', self.image.deep_x)
        ...
        #=============================strategy=============================
            ...
            if self.preturn_left:
                while abs(self.walk.get_imu()) < 58:
                    ...
                    rospy.loginfo(f'imu =  {self.walk.get_imu()}\033[K')
                self.preturn_left = False
                ...
                if self.image.deep_y < 24:
                    ...
                    if 13 > self.image.deep_x > 4 : #deep_x = dx  #normal turn 右轉 範圍越大越容易旋轉 三個地方要調整 大的數字不動
                        ...
                        if ((abs(self.walk.get_imu()) > 5) and (not self.imu_ok)) and (self.image.deep_x >= 6) :       #normal turn 右轉 數值越大 越不容易 修imu
                            ...
                            self.walk.move('imu_fix')
                            rospy.loginfo('imu_right: %s\033[K', self.walk.get_imu())
                        else:
                        ...
                        if ((abs(self.walk.get_imu()) > 5) and (not self.imu_ok)) and (self.image.deep_x <= -3.5) :   #normal turn 左轉 數值越大 越不容易 修imu   
                            # while(abs(self.walk.get_imu()) > 5):
                            self.walk.move('imu_fix')
                            rospy.loginfo('imu_left: %s\033[K', self.walk.get_imu())
                        else:
                            self.walk.move('dx_turn')
                                ...
                                if abs(self.walk.get_imu()) < 2:        #轉頭策略
                                    ...
                                    #     self.deep_x = 0
                                        ...
                                        while abs(self.walk.get_imu()) < 20:    
                                            self.walk.move('turn_left_for_wall')
                                            rospy.loginfo('dx: %s\033[K', self.image.deep_x)
                                            rospy.loginfo('deep_sum_l: %s\033[K', self.image.deep_sum_l)
                                        self.imu_ok = True
                                    elif  self.image.deep_sum_l < self.image.deep_sum_r :
                                        while abs(self.walk.get_imu()) < 20:    
                                            self.walk.move('turn_right_for_wall')
                                            rospy.loginfo('dx: %s\033[K', self.image.deep_x)
                                            rospy.loginfo('deep_sum_r: %s\033[K', self.image.deep_sum_r)
                                        self.imu_ok = True
                                else:
                                    pass
        ...
        if not send.is_start :
            ...
            self.image.calculate()
            rospy.loginfo('right_deep : %s\033[K', self.image.right_deep )
            rospy.loginfo('center: %s\033[K', self.image.center_deep )
